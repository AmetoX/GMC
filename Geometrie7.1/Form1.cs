...
namespace Geometrie7._1
{
    public partial class Form1 : Form
    {
        ...
        private void panel1_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            Point punctnou = new Point(e.X, e.Y);
            Pen penita = new Pen(Color.Blue, 3);
            g.DrawString(contor.ToString(), new Font(FontFamily.GenericSansSerif, 10), new SolidBrush(Color.Black), punctnou.X - 20, punctnou.Y - 20);
            contor++;
            g.DrawEllipse(pen, punctnou.X - 2, punctnou.Y - 2, 4, 4);
            if (points.Count != 0)
            {
                g.DrawLine(penita, punctnou, points[points.Count - 1]);
            }
            points.Add(punctnou);
        ...
        private void buttonFinishUp_Click(object sender, EventArgs e)
        {
            Graphics g = panel1.CreateGraphics();
            Pen penita = new Pen(Color.Blue, 3);
            g.DrawLine(penita, points[0], points[points.Count - 1]);
        }
            ...
            while (n > 3)
            {
                for (int i = 0; i < n - 2; i++)
                {
                    if (EsteDiagonala(puncteTriangulare, i, i + 2))
                    {
                        ...
                        newLabelY += 20;
                        g.DrawLine(grayPen, puncteTriangulare[i], puncteTriangulare[i + 2]); //deseneaza diagonala
        ...
        {
            Graphics g = panel1.CreateGraphics();
            Pen[] pens = new Pen[] { new Pen(Color.Magenta, 3), new Pen(Color.Orange, 3), new Pen(Color.Red, 3) };
            ...
            for (int i = triangles.Count - 1; i >= 0; i--)
            {
                ...
                if (colored1 == -1 && colored2 == -1 && colored3 == -1)
                {
                    ...
                    g.DrawRectangle(pens[0], points[triangles[i].Item1].X - 3, points[triangles[i].Item1].Y - 3, 8, 8);
                    varfuriMarcate.Add(new Tuple<int, int>(triangles[i].Item2, 1));
                    g.DrawRectangle(pens[1], points[triangles[i].Item2].X - 3, points[triangles[i].Item2].Y - 3, 8, 8);
                    varfuriMarcate.Add(new Tuple<int, int>(triangles[i].Item3, 2));
                    g.DrawRectangle(pens[2], points[triangles[i].Item3].X - 3, points[triangles[i].Item3].Y - 3, 8, 8);
                }
                else if (colored1 == -1)
                {
                    varfuriMarcate.Add(new Tuple<int, int>(triangles[i].Item1, NextColor(colored2, colored3)));
                    g.DrawRectangle(pens[NextColor(colored2, colored3)], points[triangles[i].Item1].X - 3, points[triangles[i].Item1].Y - 3, 8, 8);
                }
                else if (colored2 == -1)
                {
                    varfuriMarcate.Add(new Tuple<int, int>(triangles[i].Item2, NextColor(colored1, colored3)));
                    g.DrawRectangle(pens[NextColor(colored1, colored3)], points[triangles[i].Item2].X - 3, points[triangles[i].Item2].Y - 3, 8, 8);
                }
                else if (colored3 == -1)
                {
                    varfuriMarcate.Add(new Tuple<int, int>(triangles[i].Item3, NextColor(colored1, colored2)));
                    g.DrawRectangle(pens[NextColor(colored1, colored2)], points[triangles[i].Item3].X - 3, points[triangles[i].Item3].Y - 3, 8, 8);
                }
            }
        }
        ...
        private void buttonArie_Click(object sender, EventArgs e)
        {
            ...
            textBox1.Text = arieTotala.ToString();
        }
